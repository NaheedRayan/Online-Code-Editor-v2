...
// expose this function to our app.js using module.exports
module.exports = function (passport) {
    ...
    passport.deserializeUser(function (id, done) {
        ...
        result.then(result => {
            ...
            console.log(err)
            return done(null, false, req.flash("error", "Something went wrong")); // create the Message and save it to session as flashdata
        })
    });
    ...
    passport.use(
        ...
        new LocalStrategy({
            ...
            const db = DbService.getServiceInstance();//getting db instance
            ...
            if (req.body.username && req.body.email) {
                ...
                Promise.all([result1, result2]).then(data => {
                        ...
                        if (data[0].length && data[1].length == 0) {
                            return done(null, false, req.flash("error", "The username exists!!")); // create the Message and save it to session as flashdata
                        } else if (data[0].length == 0 && data[1].length) {
                            return done(null, false, req.flash("error", "The email exists!!")); // create the Message and save it to session as flashdata
                        } else if (data[0].length && data[1].length) {
                            return done(null, false, req.flash("error", "The username and email already exists!!")); // create the Message and save it to session as flashdata
                        } else if (req.body.password != req.body.confirm_password) {
                            return done(null, false, req.flash("error", "Passwords do not match")); // create the Message and save it to session as flashdata
                        } else {
                            let val = [
                            ...
                            result
                                .then(data => {
                                    // making a newUser object for passing it
                                    let newUser = {
            ...
            function (req, username_or_email, password, done) { // callback with email and password from our form
                ...
                result.then(result => {
                    ...
                    if (result.length) {
                        ...
                        return done(null, false, req.flash("error", "Invalid Username or Password")); // create the Message and save it to session as flashdata
                    }
                }).catch(err => {
                    console.log(err)
                    return done(null, false, req.flash("error", "Something went wrong")); // create the Message and save it to session as flashdata
